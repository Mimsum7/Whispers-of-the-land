/*
  ...
  1. Database Functions
    ...
    - Create or update the uid function
    - Ensure update triggers are working
  ...
  3. Triggers
    - Ensure new user trigger is properly configured
*/
...
-- Ensure stories policies are working correctly
...
CREATE POLICY "Contributors can read own stories"
  ...
  FOR SELECT
  TO public
  USING (contributor_email = COALESCE(((current_setting('request.jwt.claims'::text, true))::json ->> 'email'::text), ''::text));
